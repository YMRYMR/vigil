...
pub fn apply() -> (Vec<String>, Vec<String>) {
    #[cfg(windows)]
    {
        ...
        match platform::disable_usb_storage() {
            Ok(changed) => {
                if changed { state.usb_disabled = true; applied.push("USB storage disabled".into()); }
            }
        ...
        audit::record("quarantine_extended_apply", if warnings.is_empty() { "success" } else { "partial" }, json!({"applied": applied, "warnings": warnings, "paused_tasks": state.paused_tasks}));
        (applied, warnings)
    }
    #[cfg(not(windows))]
    ...
    #[cfg(windows)]
    {
        ...
        if !state.paused_tasks.is_empty() {
            match platform::resume_scheduled_tasks(&state.paused_tasks) {
                Ok(restored) => {
                    if restored > 0 { cleared.push(format!("{} scheduled task(s) resumed", restored)); }
                }
                Err(err) => warnings.push(format!("scheduled-task resume failed: {err}")),
            }
        }
        let _ = std::fs::remove_file(state_path());
        audit::record("quarantine_extended_clear", if warnings.is_empty() { "success" } else { "partial" }, json!({"cleared": cleared, "warnings": warnings }));
        (cleared, warnings)
    }
    #[cfg(not(windows))]
...
fn save_state(state: &State) -> Result<(), String> {
    ...
    let text = serde_json::to_string_pretty(state).map_err(|e| format!("failed to serialise quarantine state: {e}"))?;
...
mod platform {
    ...
    pub fn disable_usb_storage() -> Result<bool, String> {
        ...
        let (key, _) = hklm.create_subkey(r"SYSTEM\CurrentControlSet\Services\USBSTOR").map_err(|e| format!("failed to open USBSTOR key: {e}"))?;
        let current = key.get_value::<u32, _>("Start").unwrap_or(3);
        if current == 4 { return Ok(false); }
        key.set_value("Start", &4u32).map_err(|e| format!("failed to set USBSTOR Start=4: {e}"))?;
        Ok(true)
    }
    pub fn restore_usb_storage() -> Result<(), String> {
        let hklm = RegKey::predef(HKEY_LOCAL_MACHINE);
        let (key, _) = hklm.create_subkey(r"SYSTEM\CurrentControlSet\Services\USBSTOR").map_err(|e| format!("failed to open USBSTOR key: {e}"))?;
        key.set_value("Start", &3u32).map_err(|e| format!("failed to set USBSTOR Start=3: {e}"))
    }
    pub fn pause_scheduled_tasks() -> Result<Vec<String>, String> {
        let output = schtasks().args(["/Query", "/FO", "CSV", "/NH"]).output().map_err(|e| format!("failed to run schtasks /Query: {e}"))?;
        if !output.status.success() { return Err(format!("schtasks /Query failed with status {}", output.status)); }
        ...
        for line in text.lines() {
            ...
            let Some(task_name) = cols.next() else { continue; };
            if task_name.starts_with("\\Microsoft\\") || task_name.eq_ignore_ascii_case("TaskName") { continue; }
            let status = schtasks().args(["/Change", "/TN", task_name, "/Disable"]).status().map_err(|e| format!("failed to disable {task_name}: {e}"))?;
            if status.success() { paused.push(task_name.to_string()); }
        }
        Ok(paused)
    }
    pub fn resume_scheduled_tasks(tasks: &[String]) -> Result<usize, String> {
        ...
        for task in tasks {
            let status = schtasks().args(["/Change", "/TN", task, "/Enable"]).status().map_err(|e| format!("failed to enable {task}: {e}"))?;
            if status.success() { restored += 1; }
        }
        Ok(restored)
    }
    fn schtasks() -> Command {
