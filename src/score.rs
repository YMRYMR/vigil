...
pub fn score(input: &ScoreInput<'_>, cfg: &Config) -> (u8, Vec<String>, Vec<String>) {
    ...
    let trusted: std::collections::HashSet<String> = cfg.trusted_processes.iter().map(|s| s.to_lowercase()).collect();
    let lolbins: std::collections::HashSet<String> = cfg.lolbins.iter().map(|s| s.to_lowercase()).collect();
    ...
    if !input.path.is_empty() {
        ...
        for frag in &cfg.suspicious_path_fragments {
            if path_upper.contains(&frag.to_uppercase()) {
                ...
                attack_tags.push("T1204 User Execution / Suspicious Drop Location (heuristic)".into());
                break;
            }
        }
    }
    ...
    if lolbins.contains(input.name) {
        ...
        reasons.push(format!("System binary making network connection: {} (living-off-the-land)", input.name));
    ...
    for (parent_name_lc, _pid) in input.ancestors.iter() {
        ...
        for (bad_parent, bad_children) in SUSPICIOUS_PARENTS {
            if parent_key == *bad_parent && bad_children.contains(&input.name) {
                s = s.saturating_add(3);
                reasons.push(format!("Suspicious parent process: {} spawned {}", parent_name_lc, input.name));
                attack_tags.push("T1204 User Execution / Suspicious Parent Chain (heuristic)".into());
                break;
            }
        }
    }
    ...
    if input.recently_dropped {
        s = s.saturating_add(3);
        reasons.push("Executable was just dropped into a watched directory (possible dropper)".into());
        attack_tags.push("T1105 Ingress Tool Transfer (heuristic)".into());
    }

    if let Some(country) = input.country {
        if !cfg.allowed_countries.is_empty() && !cfg.allowed_countries.iter().any(|a| a.eq_ignore_ascii_case(country)) {
            s = s.saturating_add(2);
    ...
    if input.long_lived && !is_trusted {
        s = s.saturating_add(2);
        reasons.push(format!("Long-lived connection held open past {}s by untrusted process", cfg.long_lived_secs));
    ...
    const DNS_NATIVE: &[&str] = &["svchost", "dnscache", "dns", "systemd-resolved", "resolved", "named", "unbound", "dnsmasq"];
    if input.remote_port == 53 && !DNS_NATIVE.contains(&input.name) && !is_trusted {
        s = s.saturating_add(2);
        reasons.push(format!("DNS query from non-DNS process: {} (possible DNS tunneling)", input.name));
    ...
    if input.remote_port > 0 {
        if malware_ports.contains(&input.remote_port) {
            ...
            reasons.push(format!("Connection to known malware/C2 port {}", input.remote_port));
    ...
    let script_host_additions = detection_depth::inspect_script_host(input.name, input.command_line);
    ...
    detection_depth::inspect_behavioural_baseline(input.name, input.baseline_signal)
        .merge_into(&mut s, &mut reasons, &mut attack_tags);
    detection_depth::inspect_parent_token_anomaly(input.name, input.ancestors, input.proc_user, input.parent_user)
        .merge_into(&mut s, &mut reasons, &mut attack_tags);
...
mod tests {
    ...
    fn input<'a>(name: &'a str, path: &'a str, remote_ip: &'a str, remote_port: u16, status: &'a str) -> ScoreInput<'a> {
        ScoreInput {
            name,
            path,
            publisher: "",
    ...
    fn listen_no_remote_is_zero() {
        ...
        assert_eq!(s, 0); assert!(r.is_empty()); assert!(tags.is_empty());
    }

    #[test]
    fn script_host_adds_reason_and_tag() {
        let mut inp = input("powershell", r"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe", "1.2.3.4", 443, "ESTABLISHED");
    ...
    fn mature_baseline_deviation_adds_score() {
        ...
        inp.baseline_signal = BaselineSignal { mature: true, new_remote: true, new_port: false, new_country: false, observations: 20 };
