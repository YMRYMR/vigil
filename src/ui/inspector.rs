            ...
            if response_enabled {
                ui.horizontal_wrapped(|ui| {
                    if quarantine_active {
                        ...
                        let clear_btn = clear_btn
                            .on_hover_cursor(egui::CursorIcon::PointingHand)
                            .on_hover_text("Restore the network and undo the selected process containment steps where possible.");
                        if clear_btn.clicked() {
                            action = Some(Action::ClearQuarantineProfile);
                        }
                    } else {
                        let quarantine_btn = ui.add_enabled(quarantine_ready, danger_btn("Quarantine profile"));
                        let quarantine_btn = if quarantine_ready {
                            quarantine_btn
                                .on_hover_cursor(egui::CursorIcon::PointingHand)
                                .on_hover_text("Initial quarantine preset: isolate the network, block the executable path, and suspend the process when available.")
                        } else {
                            quarantine_btn.on_hover_text("Quarantine profile requires administrator privileges and a real PID or known executable path.")
                        };
                        if quarantine_btn.clicked() {
                            action = Some(Action::QuarantineProfile);
                        }
                    }
                });
            ...
            ui.horizontal_wrapped(|ui| {
                if domain_blocked {
                    ...
                    let domain_btn = if domain_enabled && domain_target.is_some() {
                        ...
                        domain_btn.on_hover_cursor(egui::CursorIcon::PointingHand).on_hover_text(format!("Redirect {host} to the local machine through the Windows hosts file."))
                    } else {
                        domain_btn.on_hover_text("Block domain requires administrator privileges and a selected connection with a resolved hostname.")
                    };
                ...
                let kill_conn_resp = if connection_kill_enabled {
                    kill_conn_resp.on_hover_cursor(egui::CursorIcon::PointingHand).on_hover_text("Immediately terminate the selected live TCP connection.")
                } else {
                    kill_conn_resp.on_hover_text("Select an IPv4 TCP connection with a killable state while Vigil is running as administrator.")
                };
                ...
                if process_suspended {
                    ...
                    let suspend = if suspend_enabled {
                        suspend.on_hover_cursor(egui::CursorIcon::PointingHand).on_hover_text("Freeze the process while you investigate. Use Resume process to continue it later.")
                    } else {
                        suspend.on_hover_text("Suspension requires a real PID, administrator privileges, and the Windows active-response backend.")
                    };
...
fn kv(ui: &mut Ui, key: &str, val: &str) {
    ui.horizontal(|ui| {
        ui.add_sized([88.0, 16.0], egui::Label::new(RichText::new(key).color(theme::TEXT3).size(11.0)));
...
fn kv_mono(ui: &mut Ui, key: &str, val: &str) {
    ui.horizontal(|ui| {
        ui.add_sized([88.0, 16.0], egui::Label::new(RichText::new(key).color(theme::TEXT3).size(11.0)));
        ui.add(egui::Label::new(RichText::new(val).color(theme::TEXT).monospace().size(10.5)).wrap());
    });
    ui.add_space(2.0);
}
...
fn score_badge(ui: &mut Ui, score: u8) {
    ...
    ui.label(RichText::new(format!(" {score:>2} ")).color(fg).background_color(bg).monospace().size(12.0).strong());
}

fn chip(ui: &mut Ui, text: &str) {
    ui.label(RichText::new(format!(" {text} ")).color(theme::TEXT2).background_color(theme::SURFACE3).size(10.0).strong());
}

fn blocked_status_row(ui: &mut Ui, label: &str, remaining: Option<std::time::Duration>, hover_prefix: &str, unblock_action: Action, action: &mut Option<Action>) {
    ui.add_space(4.0);
    ui.horizontal_wrapped(|ui| {
        ...
        let unblock = unblock.on_hover_cursor(egui::CursorIcon::PointingHand).on_hover_text(format!("{}.", hover_prefix));
        if unblock.clicked() { *action = Some(unblock_action); }
    });
}
...
fn accent_btn(text: &str) -> egui::Button<'_> {
    egui::Button::new(RichText::new(text).color(theme::ACCENT).size(11.5)).fill(theme::ACCENT_BG).stroke(egui::Stroke::new(1.0, theme::ACCENT)).corner_radius(6.0)
}

fn block_btn(preset: active_response::DurationPreset, text: &str) -> egui::Button<'_> {
    let (fg, bg, border) = match preset {
        active_response::DurationPreset::OneHour => (theme::ACCENT, theme::ACCENT_BG, theme::ACCENT),
        active_response::DurationPreset::OneDay => (theme::WARN, theme::WARN_BG, theme::WARN),
        active_response::DurationPreset::Permanent => (theme::DANGER, theme::DANGER_BG, theme::DANGER),
    };
    egui::Button::new(RichText::new(text).color(fg).size(11.5)).fill(bg).stroke(egui::Stroke::new(1.0, border)).corner_radius(6.0)
}

fn muted_btn(text: &str) -> egui::Button<'_> {
    egui::Button::new(RichText::new(text).color(theme::TEXT2).size(11.5)).fill(theme::SURFACE2).stroke(egui::Stroke::new(1.0, theme::BORDER)).corner_radius(6.0)
}

fn danger_btn(text: &str) -> egui::Button<'_> {
    egui::Button::new(RichText::new(text).color(theme::DANGER).size(11.5)).fill(theme::DANGER_BG).stroke(egui::Stroke::new(1.0, theme::DANGER)).corner_radius(6.0)
}
...
fn format_remaining(duration: std::time::Duration) -> String {
    ...
    let secs = secs % 60;
    if hours > 0 { format!("{hours:02}:{mins:02}:{secs:02}") } else { format!("{mins:02}:{secs:02}") }
}
