    ...
    egui::ScrollArea::vertical().id_salt("help_scroll").show(ui, |ui| {
        ...
        if ui.available_width() > 700.0 {
            ...
            card(ui, "Integrity and provenance", |ui| {
                body(
                    ui,
                    "Vigil requires .sha256 sidecars for configured blocklists and response-rule files, records first-seen and changed hashes for those operator-managed inputs, logs startup integrity results, and quarantines corrupted forensic artifacts when their manifests no longer match.",
                );
            });
