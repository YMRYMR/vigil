...
const KEEP_STATUSES: &[&str] = &[
    "ESTABLISHED", "LISTEN", "SYN_SENT", "SYN_RECV", "CLOSE_WAIT",
];
...
fn windows_tcp() -> Vec<RawConn> {
    ...
    unsafe {
        ...
        let mut size: u32 = 0;
        GetExtendedTcpTable(None, &mut size, false, 2 /*AF_INET*/, TCP_TABLE_OWNER_PID_ALL, 0);
        ...
        for row in rows {
            ...
            out.push(RawConn {
                ...
                local_port,
                remote_ip:   if is_listen { String::new() } else { remote_ip.to_string() },
...
fn platform_poll() -> Vec<RawConn> {
    let mut out = Vec::new();
    out.extend(linux_parse_proc("/proc/net/tcp",  false));
    out.extend(linux_parse_proc("/proc/net/tcp6", true));
    out
}

#[cfg(target_os = "linux")]
fn linux_parse_proc(path: &str, ipv6: bool) -> Vec<RawConn> {
    use std::fs;
    ...
    for line in content.lines().skip(1) {
        ...
        let pid = fields[11].parse::<u32>().unwrap_or(0);
...
fn parse_linux_addr(s: &str, ipv6: bool) -> (String, u16) {
    ...
    if parts.len() != 2 { return (String::new(), 0); }
        ...
        if raw.len() == 32 {
            ...
            for (i, chunk) in raw.as_bytes().chunks(8).enumerate() {
                if let Ok(s) = std::str::from_utf8(chunk) {
                    words[i] = u32::from_str_radix(s, 16)
                        .map(u32::from_le)
                        .unwrap_or(0);
                }
            }
            Ipv6Addr::from(unsafe { std::mem::transmute::<[u32; 4], [u8; 16]>(words) })
                .to_string()
        } else {
            String::new()
        }
...
fn macos_netstat() -> Vec<RawConn> {
    ...
    for line in text.lines().skip(2) {
        ...
        if !fields[0].starts_with("tcp") { continue; }

        let status = fields[5].to_uppercase();
        let pid: u32 = fields.last()
