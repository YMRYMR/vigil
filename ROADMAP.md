...
- [x] **Process scan on creation foundations** — `src/yara_scan.rs` compiles bundled plus verified operator-local rules into a bounded background worker; `src/monitor/mod.rs::process_conn()` overlays cached matches into the live score flow, and follow-up events carry bounded `YARA rule: <name>` reasons without blocking first-seen connection enrichment. The current execution contract lives in `docs/YARA-PROCESS-SCAN.md`.
- [x] **Memory region scan foundations** — `src/forensics.rs` scans Vigil-captured, opt-in process-dump artifacts with the same trusted YARA rule boundary, persists results under `target_kind = "process_dump"`, and audits `matched`, `clean`, `skipped`, and `error` outcomes. Broader live-memory acquisition remains future work; the current contract lives in `docs/YARA-MEMORY-SCAN.md`.
