...
`require_advisory_mitigation_guidance` is intentionally narrow. Today it matches only when the same high-confidence advisory reason includes `mitigation guidance available`, which Vigil emits only when the matched advisory record already carries non-empty mitigation, remediation, workaround, guidance, or solution text/URLs in the protected advisory cache, or a clearly guidance- or remediation-tagged reference URL such as `Mitigation`, `Remediation`, `Fix`, `Patch`, `Update`, `Upgrade`, `Workaround`, `Guidance`, or `Solution`. The tag matcher also normalizes singular/plural forms, simple `-`, `_`, and `/` separators, and split `Work Around` spellings, so explicit forms like `Workarounds`, `vendor_workaround`, `vendor/solution`, or `Work Around` still count only through their tagged remediation token. Vigil now also ignores bare fixed-version tokens like `124.0.6367.99`, so version-only fields do not get mistaken for operator guidance.

`require_advisory_vendor_mitigation_guidance` is intentionally narrower. Today it matches only when the same high-confidence advisory reason also includes `vendor guidance available`, which Vigil emits only when the matched advisory record already has a non-empty guidance- or remediation-tagged reference URL and that same reference is also tagged as vendor-authored material such as `Vendor Advisory`, `Vendor Bulletin`, `Vendor Notice`, `Vendor Mitigation`, `Vendor Remediation`, `Vendor Fix`, `Vendor Patch`, `Vendor Update`, `Vendor Upgrade`, `Vendor Workaround`, `Vendor Guidance`, or `Vendor Solution`. Like the remediation matcher, it also normalizes singular/plural forms, simple `-`, `_`, and `/` separators, and split `Vendor Work Around` spellings. It does not infer authorship from arbitrary domains, free-form mitigation text, or untagged links.
